...
export const msg = {
  ...
  astap: {
    ...
    noSolutionFov: (lang: ServerLang, fov: string) =>
      m(lang, ` (dernière tentative : FOV ${fov}°)`, ` (last attempt: FOV ${fov}°)`),
    ...
    noWcsFile: (lang: ServerLang) =>
      m(lang, "ASTAP n'a pas produit de fichier WCS", 'ASTAP did not produce a WCS file'),
  ...
  api: {
    missingFile: (lang: ServerLang) => m(lang, 'Fichier manquant', 'Missing file'),

    noFile: (lang: ServerLang) => m(lang, 'Aucun fichier fourni', 'No file provided'),
    ...
    notEnoughCatalogStars: (lang: ServerLang) =>
      m(
        lang,
        "Pas assez d'étoiles du catalogue dans le champ",
        'Not enough catalog stars in field',
      ),

    solveCanceled: (lang: ServerLang) => m(lang, 'Résolution annulée', 'Solve canceled'),
