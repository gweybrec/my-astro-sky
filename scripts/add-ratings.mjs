...
const ASTRO_DB_URL =
  'https://raw.githubusercontent.com/bterrier/astrogenerator/master/dbastrogenerator';
...
const HERSCHEL_400 = new Set([
  ...[
    40, 129, 136, 157, 185, 205, 225, 246, 247, 253, 288, 300, 362, 380, 404, 436, 457, 488, 524,
    559, 578, 584, 596, 613, 615, 628, 636, 650, 654, 663, 720, 752, 772, 779, 869, 884, 891, 908,
    936, 1003, 1023, 1027, 1084, 1245, 1342, 1407, 1444, 1501, 1502, 1513, 1514, 1528, 1535, 1545,
    1664, 1788, 1817, 1857, 1907, 1931, 1964, 2022, 2024, 2126, 2169, 2185, 2194, 2204, 2215, 2244,
    2261, 2281, 2301, 2324, 2359, 2360, 2362, 2371, 2403, 2419, 2420, 2438, 2440, 2479, 2539, 2549,
    2655, 2683, 2742, 2768, 2775, 2782, 2841, 2857, 2903, 2950, 2964, 2974, 2976, 3003, 3034, 3053,
    3077, 3079, 3115, 3147, 3166, 3169, 3184, 3190, 3193, 3198, 3226, 3227, 3242, 3245, 3277, 3294,
    3344, 3377, 3379, 3384, 3412, 3432, 3489, 3504, 3521, 3547, 3593, 3607, 3610, 3613, 3615, 3628,
    3631, 3640, 3675, 3686, 3726, 3756, 3810, 3877, 3893, 3898, 3941, 3945, 3962, 4026, 4038, 4039,
    4085, 4088, 4111, 4143, 4145, 4151, 4157, 4179, 4203, 4214, 4216, 4245, 4251, 4258, 4261, 4273,
    4274, 4278, 4281, 4346, 4350, 4361, 4365, 4371, 4374, 4379, 4387, 4388, 4394, 4414, 4419, 4429,
    4435, 4438, 4442, 4449, 4450, 4459, 4473, 4477, 4478, 4490, 4494, 4517, 4526, 4527, 4535, 4540,
    4546, 4548, 4550, 4559, 4564, 4565, 4570, 4594, 4596, 4608, 4621, 4636, 4643, 4654, 4656, 4660,
    4664, 4666, 4689, 4697, 4699, 4710, 4725, 4753, 4754, 4762, 4772, 4900, 4995, 5005, 5012, 5033,
    5055, 5195, 5248, 5322, 5363, 5364, 5466, 5473, 5474, 5557, 5587, 5631, 5634, 5676, 5701, 5746,
    5762, 5775, 5813, 5838, 5850, 5866, 5879, 5907, 5982, 6118, 6207, 6210, 6217, 6229, 6369, 6384,
    6445, 6503, 6520, 6522, 6528, 6540, 6543, 6567, 6569, 6572, 6633, 6638, 6643, 6645, 6664, 6712,
    6716, 6755, 6760, 6781, 6802, 6818, 6819, 6826, 6830, 6834, 6838, 6882, 6885, 6888, 6905, 6939,
    6940, 6946, 6951, 6960, 6992, 7000, 7006, 7009, 7027, 7062, 7086, 7129, 7139, 7142, 7177, 7217,
    7243, 7293, 7331, 7380, 7448, 7479, 7510, 7606, 7619, 7626, 7640, 7662, 7686, 7789, 7790,
  ].map((n) => 'NGC' + n),
]);
...
function interetToScore(interet) {
  switch (interet) {
    case 1:
      return 1; // No interest
    case 2:
      return 2; // Little interest
    case 3:
      return 3; // Interesting
    case 4:
      return 5; // Remarkable → 5 (skip 4 to create spread)
    default:
      return null;
  }
}
...
function typeDifficultyHeuristic(type, mag, emissionLines) {
  switch (type) {
    ...
    case 'GC':
      return 1;
    case 'PN':
      return mag !== null && mag > 12 ? 3 : 2;
    case 'EN':
      return emissionLines ? 2 : 3;
    case 'RN':
      return 4;
    case 'SNR':
      return 4;
...
async function main() {
  ...
  try {
    ...
    console.warn(
      `  WARNING: Could not fetch OpenNGC CSV (${e.message}). Galaxy difficulty will use fallback.`,
    );
  }
  ...
  const nLinked = computeContainers(dsoJson.data, {
    idxId,
    idxRa,
    idxDec,
    idxMajAxis,
  ...
  console.log('\n─── Source coverage ───');
  console.log(
    `  Both A+B: ${nBoth}, A only: ${nSourceA}, B only: ${nSourceB}, Heuristic: ${nHeuristic}`,
  );
...
function computeRenderPriority(data, { idxRa, idxDec, idxMag, idxRating }) {
  const n = data.length;
  const importanceRank = data
    .map((_, i) => i)
    .sort((a, b) => {
      const ra = data[a][idxRating] ?? 0,
        rb = data[b][idxRating] ?? 0;
