<template>
  <BaseModal
    :title="t('settings.solverSection')"
    size="wide"
  ...
  >
    ...
    <div class="settings-field settings-field--tight">
      ...
      <div class="probe-status-line">
        <span v-if="astapProbe.state === 'idle'" class="text-dim">{{
          t('settings.probeNotConfigured')
        }}</span>
        ...
        <span v-else-if="astapProbe.state === 'error'" class="probe-error"
          >✗ {{ astapProbe.summary }}</span
        >
      </div>
    </div>
    ...
    <div class="settings-field settings-field--tight">
      ...
      <input
        ...
        class="settings-field-input"
        v-model="sfPath"
        @blur="probeSolveField"
        @keydown.enter="probeSolveField"
      />
      <div v-if="isWindows" class="settings-checkbox-row !mt-1 !mb-0">
        <label class="settings-checkbox-label">
      ...
      <!-- always rendered — prevents layout shift -->
      <div class="probe-status-line">
        <span v-if="sfProbe.state === 'idle'" class="text-dim">{{
          t('settings.probeNotConfigured')
        }}</span>
        ...
        <span v-else-if="sfProbe.state === 'error'" class="probe-error"
          >✗ {{ sfProbe.summary }}</span
        >
      </div>
    </div>
    ...
    <div class="settings-field">
      ...
      <input
        ...
        v-model="dataDir"
        @blur="probeDataDir"
        @keydown.enter="probeDataDir"
      />
      <!-- always rendered — prevents layout shift -->
      <div class="probe-status-line">
        <span v-if="dataDirProbe.state === 'idle'" class="text-dim">{{
          t('settings.probeNotConfigured')
        }}</span>
        ...
        <details v-else-if="dataDirProbe.state === 'ok'">
          ...
          <pre class="solve-error-pre probe-pre-ok probe-dir-listing">{{
            dataDirProbe.detail
          }}</pre>
        </details>
    ...
    <div class="settings-field">
      ...
      <div class="text-dim-xs" :class="{ hidden: !apiKeySet }">
        {{ t('settings.astrometryApiKeySet') }}
      </div>
    </div>
    ...
    <div class="settings-btn-row">
      <button class="btn-confirm btn-action--full" @click="onSave" :disabled="saving">
        {{ t('settings.saveSettings') }}
      </button>
      <span class="settings-save-status" :class="{ hidden: !saveSuccess }">{{
  ...
  <Teleport to="body">
    <div v-if="activeTooltip !== null" class="hints-info-tooltip" :style="tooltipStyle" @click.stop>
  ...
  try {
    ...
    if (data.ok) {
      ...
      astapProbe.value = {
        state: 'error',
        summary,
        detail: [data.stdout, data.stderr].filter(Boolean).join('\n'),
      };
    }
  } catch (err: any) {
    if (gen !== astapGen) return;
    astapProbe.value = {
      state: 'error',
      summary: t('settings.astapProbeError'),
...
async function probeSolveField() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      const summary =
        data.code === -1 ? t('settings.probePathNotFound') : t('settings.sfProbeError');
      sfProbe.value = {
        state: 'error',
        summary,
        detail: [data.stdout, data.stderr].filter(Boolean).join('\n'),
    ...
    sfProbe.value = {
      state: 'error',
      summary: t('settings.sfProbeError'),
      detail: err.message ?? String(err),
    };
  }
}
...
async function probeDataDir() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      const summary =
        data.code === -1 ? t('settings.probePathNotFound') : t('settings.dataDirProbeError');
    ...
    dataDirProbe.value = {
      state: 'error',
      summary: t('settings.dataDirProbeError'),
      detail: err.message ?? String(err),
    };
  }
}
...
const tooltipText = computed(() => {
  switch (activeTooltip.value) {
    case 'astapPath':
      return t('settings.astapPathTooltip');
    case 'sfPath':
      return t('settings.solveFieldPathTooltip');
    case 'maxParallel':
      return t('settings.maxParallelSolvesTooltip');
    case 'apiKey':
      return t('settings.astrometryApiKeyTooltip');
    case 'dataDir':
      return t('settings.astrometryDataDirTooltip');
    case 'astap':
      return t('settings.wslAstapTooltip');
    case 'sf':
      return t('settings.wslSolveFieldTooltip');
    default:
      return '';
  }
});
