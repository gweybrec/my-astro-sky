<template>
  <div class="sky-export-control">
    ...
    <DropdownPanel v-model="open" :anchor-el="btnRef" align-right min-width="200px">
      <div class="px-5 pt-3 pb-1 text-micro uppercase text-[var(--text-label)]">{{ t('settings.exportView.scopeFull') }}</div>
      <button
        type="button"
        class="search-item w-full text-left bg-transparent border-0 cursor-pointer text-[var(--text-primary)]"
        @click="run('full', 'png')"
      >{{ t('settings.exportView.png') }}</button>
      <button
        type="button"
        class="search-item w-full text-left bg-transparent border-0 cursor-pointer text-[var(--text-primary)]"
        @click="run('full', 'pdf')"
      >{{ t('settings.exportView.pdf') }}</button>

      <div class="px-5 pt-3 pb-1 text-micro uppercase text-[var(--text-label)]">{{ t('settings.exportView.scopeView') }}</div>
      <button
        type="button"
        class="search-item w-full text-left bg-transparent border-0 cursor-pointer text-[var(--text-primary)]"
        @click="run('view', 'png')"
