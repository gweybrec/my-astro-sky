...
const fakeFov = {
  adhoc: [] as any[],
  removeFrame: vi.fn((id: string) => {
    fakeFov.adhoc = fakeFov.adhoc.filter((f) => f.id !== id);
  }),
  restoreAdhocFrame: vi.fn((frame: any) => {
    fakeFov.adhoc.push(frame);
  }),
...
describe('deleteFrameWithUndo', () => {
  ...
  it('removes a free frame immediately and restores it on undo', async () => {
    const frame = {
      id: 'a1',
      setupId: 's1',
      anchor: { kind: 'sky', ra: 1, dec: 2, dsoId: null },
      rotationDeg: 0,
    };
    fakeFov.adhoc = [frame];
  ...
  it('deletes a plan (DSO) frame and re-creates it with restored framing on undo', async () => {
    ...
    expect(fakePlans.setEntryPosition).toHaveBeenCalledWith(
      'p1',
      'e-new',
      expect.objectContaining({ paDeg: 142 }),
    );
    expect(onRestored).toHaveBeenCalled();
  });
  ...
  it('re-creates a custom-location frame via addCustomEntry on undo', async () => {
    ...
    expect(fakePlans.setEntryPosition).toHaveBeenCalledWith(
      'p1',
      'e-custom',
      expect.objectContaining({ ra: 50, dec: 10 }),
    );
  });
});
