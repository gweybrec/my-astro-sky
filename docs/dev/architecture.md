...
- **Gear catalog** (`gear-catalog.ts`): Lazy-cached loaders for the four built-in equipment catalogs served from `resources/*.json` — `getTelescopes()`, `getCameras()`, `getAccessories()`, `getFilters()` — plus their `*Data` types and `*Label()` display helpers. Telescope + camera + accessory form a **gear setup**; **filters are not** — a filter is picked per photo integration row and per plan observation window. Filter-specific helpers: `filterLabel(f)` resolves the display/lookup name by precedence `astrobin_name → spcc_name → model` (unique across the catalog, enforced by `tests/unit/filters-json-schema.test.ts`); `resolveCatalogFilter(name)` / `catalogFilterColor(name)` map a stored name back to its catalog entry / `color`; `getHiddenFilterIds()` / `setHiddenFilterIds()` persist the user's "owned filters" selection in localStorage (`myastrosky-hidden-filter-ids`); `getVisibleFilterEntries()` feeds the autocomplete (catalog minus hidden). Each filter carries a `color` (`#rrggbb`) seeded by `scripts/seed-filter-colors.mjs`; `filterBadgeColors()` in `color-utils.ts` derives the badge bg/text/border from it, applied via `.filter-catalog` custom properties (POI-chip pattern). Generic band names (`Ha`, `L`…) keep their legacy `.filter-{key}` token colours — no catalog product is allowed to collide with one. **Smart telescopes** (`is_smart_telescope`) have a sealed-in sensor: never read a gear setup's `cameraId` directly — go through `resolveSetupCamera(telescope, cameras, cameraId)`, which substitutes the scope's `integrated_camera_id`. The camera picker is disabled for those scopes, so a stored `cameraId` there is meaningless (setups saved before this resolver existed carry the alphabetically-first catalog camera, which made every smart scope of a given focal length render the same wrong FOV). `tests/unit/gear-catalog-integrity.test.ts` validates the shipped catalogs: every `integrated_camera_id` resolves, sensor dimensions agree with resolution × pixel size, and each smart scope's computed FOV matches its published spec.
