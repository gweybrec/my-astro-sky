...
| A spatial/grid query (`collect`/`findAll`) dominating self-time, worse when zoomed **out**    | Radius is scale-derived (`1/scale`) so it scans a huge box of empty cells — **cap the query radius at the drawable region** (`Math.min(viewportR, capR)`). Clamping the loop to the index's populated bounds only helps if the data is compact; a projection that scatters culled objects to large coords defeats it — see T5 addendum |
