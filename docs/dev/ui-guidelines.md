...
| Role | Target class *(future)* | Current class(es) | Appearance | When |
|---|---|---|---|---|
...
|---|---|---|---|
| *(none)* | 960 px | 85 vh | Full-size modals (batch upload, light-solve) |
...
| `.targets-coord-input` | Targets-form date/coord fields *(identical to `.targets-date-input` — merge these)* |
...
| Modal sub-panel | `.modal-panel-header` | *(body inside `.modal-panel`)* | `.open` on `.modal-panel` |
| Add-photo metadata | `.metadata-toggle` | `.metadata-section` | `.open` on `.metadata-section` |
| Target card details | `<summary>` inside `<details>` | *(browser native)* | *(browser native)* |
| Batch card metadata | `.batch-meta-toggle` | *(sibling content)* | `.open` on `.batch-item-card` |
...
```css
.my-icon-btn svg { width: 16px; height: 16px; }
```
...
```css
#photo-layer.photos-frozen .photo-overlay-img { visibility: hidden; }
```
